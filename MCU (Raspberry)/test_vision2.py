...
def blackline(imag, th, wi, hi,flag):
    ...
    if i2 < 2:
        ...
        tan = tan / m /2
        midi = tan *m *2 +2*m*int(wi/2)#judge the core of line location
        if (((mid[0, 1]-mid[i2-1, 1])>40) and midi>int(wi/2)):#right turn
            flag = 0
        elif(((mid[0, 1]-mid[i2-1, 1])>40) and midi<int(wi/2)):#left turn
            flag = 1
    #flag = 0
...
#图像读取分析
def imageanalyze(imgQueue,img,imgQueuegray,flag):
    ...
    #图像分析
    flag,img3,tan = blackline(thresh1, th1, wi, hi, flag)
    queueLock.acquire()
    imgQueue.put(img3)
    queueLock.release()
    mes = 0x53
...
def motionControl(enable, imgQueue,imgQueuegray):
    ...
    for frame in camera.capture_continuous(output, format="bgr", use_video_port=True):
        img = output.array
        mes = imageanalyze(imgQueue,img,imgQueuegray,flag)
        output.truncate(0)
        send(0x53)
        send(mes)
        if enable.value == 0:
